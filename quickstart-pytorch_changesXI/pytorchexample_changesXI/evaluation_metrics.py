...
def get_data(data_file):
    """This function gets the data from the csv and builds labels using Pandas."""

    df = pd.read_csv(data_file)
    x = torch.tensor(df.iloc[:, :2].values, dtype=torch.float32)
    labels_series = (df.iloc[:, 1] > df.iloc[:, 0]).astype(float)
    labels = torch.tensor(labels_series.values, dtype=torch.float32)
...
def test_error_histogram(array_test_errors, global_test_error):
    """It creates the test error histogram with a vertical line for the 
