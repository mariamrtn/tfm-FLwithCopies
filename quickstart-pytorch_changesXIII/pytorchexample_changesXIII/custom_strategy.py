...
class custom_strategy(Strategy):     
    def __init__(self, model,  lr=0.1, batch_size=32):
        ...
        # Object to store current the labels of the current batch of data
    ...
    replies: Iterable[Message]) -> tuple[ArrayRecord | None, MetricRecord | None]:
        ...
        try:
            self.current_batch, self.current_labels = next(self.dataloader_iter)
    ...
    config: ConfigRecord, grid: Grid) -> Iterable[Message]: 
        ...
        for node_id in grid.get_node_ids():
            ...
            content = RecordDict({"arrays": arrays, "config": config}) # Here we send the current batch of data to the clients
